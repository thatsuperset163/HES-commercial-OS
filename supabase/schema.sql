...
-- Shared HQ and Work blackboard state
...
-- Requests Center (intake before job)
create table if not exists intake_requests (
  id text primary key,
  status text not null default 'new'
    check (status in (
      'new','needs_response','estimate_scheduled','waiting_on_customer','approved','declined'
    )),
...
create index if not exists intake_request_activities_request_idx on intake_request_activities (request_id, created_at desc);
