-- Phase 1: additive normalized sales foundation.
-- This migration intentionally leaves sales_workspace, commercial_prospects,
-- blackboard_workspace, and their existing policies unchanged.
--
-- Normalized application writes must be made server-side with
-- SUPABASE_SERVICE_ROLE_KEY. RLS is enabled below without anon policies.
...
create table if not exists sales_users (
  ...
  role text not null default 'sales_rep'
    check (role in ('sales_rep', 'sales_manager', 'admin')),
...
create unique index if not exists sales_users_email_unique
  on sales_users (lower(email))
  where email is not null and btrim(email) <> '';
create index if not exists sales_users_active_idx
  on sales_users (is_active, archived_at);
...
create table if not exists companies (
  ...
  notes text,
  legacy_prospect_id text unique,
  legacy_data jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  archived_at timestamptz,
  check (length(btrim(name)) > 0),
  check (jsonb_typeof(legacy_data) = 'object')
);

alter table companies
  add column if not exists assigned_user_id text
  references sales_users(id) on delete set null;
...
create table if not exists contacts (
  ...
  contact_type text not null default 'other'
    check (contact_type in ('decision_maker', 'gatekeeper', 'other')),
...
create index if not exists lead_sources_active_idx
  on lead_sources (is_active, sort_order);
...
create index if not exists opportunity_stages_order_idx
  on opportunity_stages (sort_order);
create index if not exists opportunity_stages_closed_idx
  on opportunity_stages (is_closed, is_won);
...
create table if not exists opportunities (
  ...
  lead_status text not null default 'not_contacted'
    check (lead_status in ('not_contacted', 'contacted', 'qualified', 'nurture', 'converted', 'lost')),
  priority text not null default 'medium'
    check (priority in ('low', 'medium', 'high')),
  estimated_job_value numeric(14,2)
    check (estimated_job_value is null or estimated_job_value >= 0),
  estimated_annual_value numeric(14,2)
    check (estimated_annual_value is null or estimated_annual_value >= 0),
...
alter table opportunities
  add column if not exists estimated_job_value numeric(14,2)
  check (estimated_job_value is null or estimated_job_value >= 0);
alter table opportunities
  add column if not exists estimated_annual_value numeric(14,2)
  check (estimated_annual_value is null or estimated_annual_value >= 0);
...
create table if not exists activities (
  ...
  assigned_user_id text references sales_users(id) on delete set null,
  actor_user_id text references sales_users(id) on delete set null,
  ...
  archived_at timestamptz,
  check (
    opportunity_id is not null or company_id is not null or contact_id is not null
  ),
...
begin
  if not exists (
    ...
    where conrelid = 'activities'::regclass
      and conname = 'activities_activity_type_phase1_check'
  ) then
    alter table activities add constraint activities_activity_type_phase1_check
      check (activity_type in (
        'prospect_created', 'email_sent', 'email_opened', 'email_replied',
        'phone_call', 'meeting_scheduled', 'quote_sent', 'quote_accepted',
        'quote_declined', 'job_completed', 'lost_opportunity',
        'note', 'research', 'email', 'call', 'voicemail', 'follow_up',
        'meeting', 'site_visit', 'quote', 'stage_change', 'task_created',
        'task_completed', 'attachment', 'other'
      ));
  end if;
  if not exists (
    select 1 from pg_constraint
    where conrelid = 'activities'::regclass
      and conname = 'activities_direction_phase1_check'
  ) then
    alter table activities add constraint activities_direction_phase1_check
...
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid = 'sales_tasks'::regclass
      and conname = 'sales_tasks_task_type_phase1_check'
  ) then
    alter table sales_tasks add constraint sales_tasks_task_type_phase1_check
      check (task_type in (
        'call', 'email', 'meeting', 'visit', 'site_visit',
        'quote_follow_up', 'quote', 'custom', 'other'
      ));
  end if;
end $$;
...
create index if not exists sales_tasks_open_due_idx
...
-- Seed reference data. ON CONFLICT DO NOTHING avoids overwriting later edits.
insert into sales_users (id, display_name, role)
values ('user-will', 'Will', 'sales_rep')
on conflict (id) do nothing;

insert into lead_sources (id, name, description, sort_order)
values
  ('legacy-commercial-prospects', 'Legacy Commercial Prospects', 'Imported from commercial_prospects', 0),
  ('outbound', 'Outbound', null, 10),
  ('website', 'Website', null, 20),
  ('referral', 'Referral', null, 30),
  ('networking', 'Networking', null, 40),
  ('repeat-business', 'Repeat Business', null, 50)
on conflict (id) do nothing;

insert into opportunity_stages
  (id, name, probability, sort_order, is_closed, is_won)
values
  ('prospecting', 'Prospecting', 10, 10, false, false),
  ('discovery', 'Discovery', 25, 20, false, false),
  ('site_visit', 'Site Visit', 40, 30, false, false),
  ('proposal', 'Proposal', 60, 40, false, false),
  ('negotiation', 'Negotiation', 75, 50, false, false),
  ('won', 'Won', 100, 60, true, true),
  ('lost', 'Lost', 0, 70, true, false)
...
insert into services (id, name, sort_order)
values
  ('pressure_washing', 'Pressure Washing', 10),
  ('window_cleaning', 'Window Cleaning', 20),
  ('junk_removal', 'Junk Removal', 30),
  ('soft_washing', 'Soft Washing', 40),
  ('gutter_cleaning', 'Gutter Cleaning', 50),
  ('other', 'Other', 60)
on conflict (id) do nothing;

-- Preserve named legacy sales reps and make assignments deterministic.
insert into sales_users (id, display_name, role)
select distinct
  case
    when lower(btrim(cp.sales_rep)) = 'will' then 'user-will'
    else 'legacy-user-' || md5(lower(btrim(cp.sales_rep)))
  end,
  btrim(cp.sales_rep),
  'sales_rep'
from commercial_prospects cp
where btrim(cp.sales_rep) <> ''
on conflict (id) do nothing;

-- Best-effort backfill. Legacy IDs are retained and every upsert only fills
-- the normalized record identified by that legacy ID.
insert into companies (
  id, name, assigned_user_id, industry, website, phone, address_line1, city,
  legacy_prospect_id, legacy_data, created_at, updated_at
)
select
  cp.id,
  case when btrim(cp.company_name) <> '' then cp.company_name else 'Unnamed legacy company ' || cp.id end,
  case
    when btrim(cp.sales_rep) = '' or lower(btrim(cp.sales_rep)) = 'will' then 'user-will'
    else 'legacy-user-' || md5(lower(btrim(cp.sales_rep)))
  end,
  nullif(btrim(cp.industry), ''),
  nullif(btrim(cp.website), ''),
  nullif(btrim(cp.company_phone), ''),
...
insert into contacts (
  id, company_id, full_name, job_title, email, phone, phone_ext,
  contact_type, is_primary, email_verified, decision_maker_confirmed,
...
select
  ...
  cp.id,
  cp.decision_maker,
  nullif(btrim(cp.job_title), ''),
  nullif(btrim(cp.email), ''),
  nullif(btrim(cp.phone), ''),
  nullif(btrim(cp.phone_ext), ''),
  'decision_maker',
  true,
  cp.email_verified,
  cp.decision_maker_confirmed,
  jsonb_build_object('legacy_prospect_id', cp.id, 'legacy_role', 'decision_maker'),
  cp.created_at,
  cp.updated_at
from commercial_prospects cp
where btrim(cp.decision_maker) <> ''
on conflict (id) do nothing;

insert into contacts (
  id, company_id, full_name, phone, contact_type, is_primary,
  legacy_data, created_at, updated_at
)
select
  cp.id || ':assistant',
  cp.id,
  cp.assistant_name,
  nullif(btrim(cp.assistant_phone), ''),
  'gatekeeper',
  false,
  jsonb_build_object('legacy_prospect_id', cp.id, 'legacy_role', 'assistant'),
  cp.created_at,
  cp.updated_at
from commercial_prospects cp
where btrim(cp.assistant_name) <> ''
on conflict (id) do nothing;

insert into opportunities (
  id, company_id, primary_contact_id, stage_id, lead_source_id,
  assigned_user_id, name, lead_status, priority,
  estimated_job_value, estimated_annual_value,
  first_email_at, first_call_at, next_follow_up_at, last_contact_at,
  property_notes, conversation_notes, pain_points, services_discussed,
  raw_legacy_stage, legacy_prospect_id, legacy_data, created_at, updated_at
)
select
  cp.id,
  cp.id,
  case when btrim(cp.decision_maker) <> '' then cp.id || ':primary' else null end,
  case cp.lead_status
    when 'spoke_with_dm' then 'discovery'
    when 'interested' then 'discovery'
    when 'meeting_scheduled' then 'site_visit'
    when 'site_visit' then 'site_visit'
    when 'site_visit_scheduled' then 'site_visit'
    when 'quote_sent' then 'proposal'
  ...
  'legacy-commercial-prospects',
  case
    when btrim(cp.sales_rep) = '' or lower(btrim(cp.sales_rep)) = 'will' then 'user-will'
    else 'legacy-user-' || md5(lower(btrim(cp.sales_rep)))
  end,
  case when btrim(cp.company_name) <> '' then cp.company_name || ' opportunity' else 'Legacy opportunity ' || cp.id end,
  case
    when cp.lead_status = 'won' then 'converted'
    when cp.lead_status = 'lost' then 'lost'
    when cp.lead_status = 'future_opportunity' then 'nurture'
    when cp.lead_status in ('spoke_with_dm', 'interested', 'negotiating', 'meeting_scheduled',
      'site_visit', 'site_visit_scheduled', 'quote_sent', 'proposal_sent') then 'qualified'
    when cp.lead_status in ('email_sent', 'follow_up_1', 'follow_up_2', 'follow_up_due',
      'called', 'left_voicemail') then 'contacted'
    else 'not_contacted'
  end,
  case lower(cp.priority)
    when 'high' then 'high'
    when 'low' then 'low'
    else 'medium'
  end,
  ...
  cp.last_contact_at,
  nullif(btrim(cp.property_notes), ''),
  nullif(btrim(cp.conversation_notes), ''),
  nullif(btrim(cp.pain_points), ''),
  nullif(btrim(cp.services_discussed), ''),
  cp.lead_status,
  cp.id,
  to_jsonb(cp),
  cp.created_at,
...
-- Repair ownership and stage mapping if an earlier draft already backfilled.
update companies c
...
from commercial_prospects cp
where c.legacy_prospect_id = cp.id
  and c.assigned_user_id is null;

update opportunities o
set stage_id = case cp.lead_status
  when 'spoke_with_dm' then 'discovery'
  when 'interested' then 'discovery'
  when 'meeting_scheduled' then 'site_visit'
  when 'site_visit' then 'site_visit'
...
from commercial_prospects cp
where o.legacy_prospect_id = cp.id;

insert into opportunity_services (opportunity_id, service_id)
select distinct
  cp.id,
  case service_name
    when 'exterior_maintenance' then 'other'
    else service_name
  end
from commercial_prospects cp
cross join lateral unnest(cp.services_needed) as service_row(service_name)
where service_name in (
  'pressure_washing', 'window_cleaning', 'junk_removal',
  'soft_washing', 'gutter_cleaning', 'other', 'exterior_maintenance'
)
on conflict (opportunity_id, service_id) do nothing;

-- RLS is intentionally policy-free on normalized tables. PostgreSQL table
-- owners and Supabase service-role requests bypass RLS; anon/authenticated
-- clients receive no access until explicit least-privilege policies are added.
